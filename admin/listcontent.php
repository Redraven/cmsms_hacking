...
#
#$Id: listcontent.php 8955 2013-09-16 17:41:42Z calguy1000 $
...
$xajax->register(XAJAX_FUNCTION, 'content_setmenu');
$xajax->register(XAJAX_FUNCTION, 'content_setmenuoff');
...
function content_setmenu($contentid) {
	$objResponse = new xajaxResponse();
...
function content_setmenuoff($contentid) {
	$objResponse = new xajaxResponse();

	setmenu($contentid, false);
...
function setmenu($contentid, $active = true) {
	$userid = get_userid();
...
function deletecontent($contentid) {
	...
	$access = (check_permission($userid, 'Remove Pages') &&
		...
		quick_check_authorship($contentid, $mypages))) || check_permission($userid, 'Manage All Content') || check_permission($userid, 'Modify Any Page');
...
function display_hierarchy(&$root, &$userid, $modifyall, &$users, &$menupos, &$openedArray, &$pagelist, &$image_true, &$image_set_false, &$image_set_true, &$upImg, &$downImg, &$viewImg, &$editImg, &$copyImg, &$deleteImg, &$expandImg, &$contractImg, &$mypages, &$page, $columnstodisplay, $author_allpages) {
	...
	if ($display != 'none') {
		...
		/* Menu column */
		if ($columnstodisplay['menu']) {
			$columns['menu'] = '&nbsp;';
			$txt = '';
			
			if (check_permission($userid, 'Manage All Content') && $one->Type() != 'errorpage') {
				if ($one->ShowInMenu()) {
					$txt = "<a href=\"{$thisurl}&amp;setmenuoff=" . $one->Id() . "\" onclick=\"xajax_content_setmenuoff(" . $one->Id() . ");return false;\">" . $image_set_false . "</a>";
				} else {
					$txt = "<a href=\"{$thisurl}&amp;setmenu=" . $one->Id() . "\" onclick=\"xajax_content_setmenu(" . $one->Id() . ");return false;\">" . $image_set_true . "</a>";
				}
			}
			if (!empty($txt)) {
				$columns['menu'] = $txt;
		...
		if ($columnstodisplay['copy']) {
			...
			if ($one->IsCopyable() &&
				...
				(check_ownership($userid, $one->Id()) || quick_check_authorship($one->Id(), $mypages))) || check_permission($userid, 'Manage All Content'))) {
...
function display_content_list($themeObject = null) {
	...
	if ($columnstodisplay['menu']) {
		$headoflist .= '<th scope="col" title="Denne siden blir vist i menyen." class="pagepos">Meny' . "</th>\n";
	}
	if ($columnstodisplay['active']) {
	...
	?>
		<a style="margin-left: 10px;" href="'.$thisurl.'&amp;expandall=1" onclick="xajax_content_expandall();
				return false;">
	<?php
	...
	?>
		...
		<a href="<?php echo $thisurl ?>&amp;collapseall=1" onclick="xajax_content_collapseall();
				return false;">
	<?php
...
if (isset($_GET["setmenu"])) {
	setmenu($_GET["setmenu"]);
}

// perhaps we're deactivating a page instead?
if (isset($_GET["setmenuoff"])) {
	setmenu($_GET["setmenuoff"], false);
}
